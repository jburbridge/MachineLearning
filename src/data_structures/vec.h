...
public:
	...
	//Adds a scalar to every element of this vector
	Vec& operator+=(double scalar);

	//Subtracts a scalar from every element in this vector
	Vec& operator-=(double scalar);
	...
	static Matrix outerProduct(const Vec& a, const Vec& b);

	/// Turns the vector into a 1-column matrix
	Matrix* toColumnVector() const;

	/// Take a vector and fold it into a matrix of arbitrary size.
	/// If it doesn't fit perfectly, pad the remaining values.
	Matrix* toMatrix(size_t rowLength, double padding = 0.0) const;

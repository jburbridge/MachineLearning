...
Vec* Matrix::toVec(size_t s) const
{
	if(s == 0)
		s = rows()*cols();
...
void Matrix::shuffle(Rand& r)
{
	size_t numRows = rows();

	for(size_t i = 0; i < numRows; i++)
	{
		size_t a = r.next() % numRows;
		size_t b = r.next() % numRows;
		swapRows(a, b);
	}
}
...
//static
Matrix* Matrix::pad(const Matrix& in, size_t padding, double val)
{
	Matrix* out = new Matrix(in.rows() + 2 * padding, in.cols() + 2 * padding);
	out->fill(val);
	out->copyBlock(padding, padding, in, 0, 0, in.rows(), in.cols());
	return out;
}
...
Matrix Matrix::operator+ (const Matrix& that) const
{
	if(rows() != that.rows() || cols() != that.cols())
		throw Ex("Error: Matrix dimensions much match for addition to work");
...
{
	if(rows() != that.rows() || cols() != that.cols())
		throw Ex("Error: Matrix dimensions much match for subtraction to work");

	Matrix diff(rows(), cols());

	for(size_t i = 0; i < diff.rows(); i++)
		for(size_t j = 0; j < diff.cols(); j++)
			diff[i][j] = (*this)[i][j] - that[i][j];
...
Matrix Matrix::operator*(double scalar) const
{
	...
	for(size_t i = 0; i < scaled.rows(); i++)
		for(size_t j = 0; j < scaled.cols(); j++)
			scaled[i][j] = (*this)[i][j] * scalar;

	return scaled;
}

Matrix& Matrix::subtractScalar(double scalar)
{
	for(size_t i = 0; i < rows(); i++)
		(*this)[i] -= scalar;

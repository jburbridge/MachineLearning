...
public:
	...
	/// Add two matrices to make a new one
	Matrix operator+ (const Matrix& that) const;

	/// Add a matrix to this one
	Matrix& operator+= (const Matrix& that);

	/// Subtract two matrices to make a new one
	Matrix operator- (const Matrix& that) const;
	...
	Vec* toVec(size_t s = 0) const;

	/// Swaps the two specified rows.
	void swapRows(size_t a, size_t b);

	/// Randomly shuffle this matrix
	void shuffle(Rand& r);

	/// Swaps the two specified columns (including meta-data).
	void swapColumns(size_t nAttr1, size_t nAttr2);

	//Pad all 4 sides of the matrix with this value
	static Matrix* pad(const Matrix& in, size_t padding = 0, double val = 0);

	/// Multiplies two matrices.
	static Matrix* multiply(const Matrix& a, const Matrix& b, bool transposeA, bool transposeB);

	/// Makes a scaled version of this Matrix
	Matrix operator*(double scalar) const;

	/// Multiplies all the values in this matrix by the scalar
	Matrix& operator*=(double scalar);

	//Subtracts a scalar from every value in this matrix 
	Matrix& subtractScalar(double scalar);
